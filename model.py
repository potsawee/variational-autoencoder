...
class VariationalAutoEncoder(object):
    def __init__(self):
        ...
        self.d = 200 # dimension of images
        self.c = 3 # num of channels e.g. 3 for RGB and 1 for grey scale

    def build_network(self):
        # this inputs must be normalised so that 0.0 to 1.0
        self.inputs = tf.placeholder(tf.float32, [None, self.d, self.d, self.c], name="inputs")
        ...
        with tf.variable_scope("encoder"):
            ...
            fc1 = fc_layer(flatten, num_inputs=self.num_flatten, num_outputs=512, name="fc1")
            relu_fc1 = relu_layer(fc1, name="relu_fc1")

            # Latent Space
            self.z_mean = fc_layer(relu_fc1, num_inputs=512, num_outputs=self.z_size, name="z_mean")
            self.z_std = fc_layer(relu_fc1, num_inputs=512, num_outputs=self.z_size, name="z_std")
        ...
        with tf.variable_scope("decoder"):
            ...
            # Fully connected layer
            fc1 = fc_layer(gaussian_z, num_inputs=self.z_size, num_outputs=512, name="fc1")
            relu_fc1 = relu_layer(fc1, name="relu_fc1")

            fc2 = fc_layer(relu_fc1, num_inputs=512, num_outputs=50*50*32, name="fc2")
            relu_fc2 = relu_layer(fc2, name="relu_fc2")

            # Reshape to be [None, height, width, depth]
            hidden = tf.reshape(relu_fc2, shape=[s[0], 50, 50, 32])

            # Deconvolution
            convt1 = conv_transpose_layer(hidden, output_shape=[s[0], 100, 100, 16], ksize=5, stride=2, name="convt1")
            relu_convt1 = relu_layer(convt1, name="relu_convt1")

            convt2 = conv_transpose_layer(relu_convt1, output_shape=[s[0], self.d, self.d, self.c], ksize=5, stride=2, name="convt2")
    ...
    def build_loss_function(self):
        ...
        s = tf.shape(self.inputs)
        _input = tf.reshape(self.inputs, shape=[s[0], self.d*self.d*self.c])
        _output = tf.reshape(self.output, shape=[s[0], self.d*self.d*self.c])
    ...
    def build_generator(self):
        ...
        with tf.variable_scope("decoder", reuse=True):
            # Fully connected layer
            fc1 = fc_layer(self.z_gen, num_inputs=self.z_size, num_outputs=512, name="fc1")
            relu_fc1 = relu_layer(fc1, name="relu_fc1")
            ...
            # Reshape to be [None, height, width, depth]
            hidden = tf.reshape(relu_fc2, shape=[64, 50, 50, 32])

            # Deconvolution
            convt1 = conv_transpose_layer(hidden, output_shape=[64, 100, 100, 16], ksize=5, stride=2, name="convt1")
            relu_convt1 = relu_layer(convt1, name="relu_convt1")

            convt2 = conv_transpose_layer(relu_convt1, output_shape=[64, self.d, self.d, self.c], ksize=5, stride=2, name="convt2")
